...
import { setEmpty, getImageUrl, canLoadWebp } from "~/utils/helpers";
...
import CreatePackege from "~/components/Dashboard/Package/CreatePackege";
...
const Package = (props: IProps) => {
  ...
  }, [newPackage, query, pagination, filters, sorter]);
  ...
  // const newPackage = (proxy: DataProxy, result: FetchResult<any>) => {
  //       const cachedData = proxy.readQuery<
  //       Get_PackagesQuery,
  //       Get_PackagesQueryVariables
  //     >({ query: GET_PACKAGES });
  //     const newRecord = result.data.insert_packages.returning[0];
  //     cachedData.packages.push(newRecord);
      ...
      <Row style={{ marginBottom: "20px" }}>
        <Col md={16}>
          <Button bordered onClick={() => setVisible(true)}>
            <Icon type="cloud-upload" /> Багц үүсгэх
          </Button>
        </Col>
        <Col md={8}>
          <Input.Search

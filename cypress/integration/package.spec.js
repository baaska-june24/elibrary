...
describe("Package", () => {
  ...
  it("Create package", () => {
    cy.visit("/dashboard");
    cy.url("include", "/dashboard/packages");
    cy.get(".ant-col-md-6 > .ant-menu > :nth-child(2) > a").click();
    cy.visit("/dashboard/packages");
...
export const packages = `query GET_PACKAGES {
  packages(where:{organization_id:{_eq: 19},type:{_eq:"package"}}) {
    key: id
    is_active
    id
    description
...
export const query_search = `query GET_PACKAGES {
  packages(where:{organization_id:{_eq: 19},type:{_eq:"package"}, name:{_ilike:"%test%"}}) {
    id
    name
  }
}`;
